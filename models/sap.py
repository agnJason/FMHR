...
def mc_from_psr(psr_grid, pytorchify=False, real_scale=False, zero_level=0):
    ...
    if batch_size>1:
        ...
        for i in range(batch_size):
            verts_cur, faces_cur, normals_cur, values = measure.marching_cubes_lewiner(psr_grid_numpy[i], level=0)
        ...
        try:
            verts, faces, normals, values = measure.marching_cubes_lewiner(psr_grid_numpy, level=zero_level)
        except:
            verts, faces, normals, values = measure.marching_cubes_lewiner(psr_grid_numpy)
    if real_scale:
